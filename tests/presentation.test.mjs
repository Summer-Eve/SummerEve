...
test('the result panel itself uses a skippable scroll animation and rare-card light effects',()=>{
 ...
 assert.match(appSource,/result-actions.*result-redraw.*<\/div><\/div><i class="scroll-core/s);
 assert.ok(!appSource.includes('scroll-roller'));
 assert.match(styleSource,/\.scroll-surface\{background:url\('\.\.\/assets\/scroll-reveal\.png'\)/);
 assert.match(styleSource,/\.scroll-core\{[^}]*background:url\('\.\.\/assets\/scroll-reveal\.png'\)/);
 assert.ok(!styleSource.includes('scroll-roll-left'));
 assert.ok(!styleSource.includes('scroll-roll-right'));
 assert.match(styleSource,/@keyframes scroll-sheet-unroll/);
 assert.match(styleSource,/@keyframes scroll-core-recede/);
